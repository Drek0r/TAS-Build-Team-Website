...
class AppSecure:
    ...
    def debugSession(self):
        if cfg.DEBUG:
            for key in session.keys():
                cfg.debug(f"KEY: {key} -> {session.get(key)}")

    def generateKey(keyName, session):
        if session.get(keyName):
            return session.get(keyName)

        password = cfg.getAppKey()
    ...
    def getSessionKey(self):
        ...
        cfg.debug(f"getsessionkey: {self.ses_key} : {sessionKey}")
        return sessionKey

    def clearSession(self):
        cfg.debug("Clearing Session")
        # [session.pop(key) for key in list(session.keys()) if key != '_flashes']
        session.clear()

    def clearSessionVar(self,key):
        cfg.debug(f"Clear {key}")
        session.pop(key)
    ...
    def setSessionVar(self, key, value):
        cfg.debug(f"SETTING SESSION VAR {key} : {value}")
        _sessionKey = self.getSessionKey()
        _value = value
        if not isinstance(value, str):
            if (isinstance(value, list)) or (isinstance(value, dict)):
                cfg.debug(f"json dums : {value}")
                _value = json.dumps(value)
        else:
            cfg.debug(f"SET TYPE: {type(value)}")
            _value = value
        ...
        debug(f"seting: {key} : {key} : {encrypted}")
    ...
    def getSessionVar(self, key):
        ...
        if session.get(_key):
            ...
            cfg.debug(f"Type: {type(decrypted)}")
            cfg.debug(f"get: {key} : {_key} : {decrypted}")
